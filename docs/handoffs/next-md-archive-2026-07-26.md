# NEXT.md archive — 2026-07-26 rotation

This file holds everything trimmed out of `NEXT.md` during the 2026-07-26 audit
(`docs/scratch/next-md-audit.md`, `next-md-audit-verify.md`), verbatim, in original document
order. Evidence labels (VERIFIED/INFERRED/NOT-OBSERVABLE) and both-halves framing are
preserved exactly as written in the source. Each block is tagged with its original NEXT.md
line range for traceability; those numbers are historical (as of commit `d239471`) and are a
convenience only, not a live pointer target.

Three items that were part of this archived material are **not only** archived: they are
also carried forward as a dated note at the end of `docs/14-session6-phase2-gate.md` (main.py
end-to-end startup composition; the orphan-crash recovery path; real-tree behavior) because
they are live limitations governing the next gate, not closed history — this archive is not
their authoritative home. Everything else below has no other copy; this file is that copy.
